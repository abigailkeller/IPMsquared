...
model_code <- nimbleCode({
  ...
  for (y in 2:n_year) {
    ...
    ## natural survival
    S[1, y, 1:n_size] <- survival(alpha, beta[1, y], x[1:n_size], 0, D[1, y])
  }
  ...
  for (y in 1:n_year) {
    for (t in 1:(totalt[y]-1)) {
      ...
      # Equation 7
      ## natural survival
      S[t + 1, y, 1:n_size] <- survival(alpha, beta[t + 1, y], x[1:n_size], 
                                        D[t, y], D[t + 1, y])
  ...
  for (y in 1:n_year) {
    for (t in 1:(additionalt[y, 2] + 1)) {
      beta[t, y] ~ dgamma(beta_alpha, beta_theta)
    }
  }
  ...
  for (y in 1:n_year_short) {
    for (t in additionalt[year_short[y], 1]:additionalt[year_short[y], 2]) {
      ...
      S[t + 1, year_short[y], 1:n_size] <- survival(alpha, beta[t + 1, y], 
                                                    x[1:n_size],
                                                    D[t, year_short[y]],
                                                    D[t + 1, year_short[y]])
    }
  ...
  ## natural survival
  S[1, 1, 1:n_size] <- survival(alpha, beta[1, 1], x[1:n_size], 0, D[1, 1])
  ...
  S_mc_rc[1, 1:n_size] <- survival(alpha, beta[1, 5], x[1:n_size], 
                                   D_mc_rc[1], D_mc_rc[2])
  ...
  for (t in 2:n_time_recap) { # time corresponds to marking events
    ...
    S_mc_rc[t, 1:n_size] <- survival(alpha, beta[t, 5], x[1:n_size], 
                                     D_mc_rc[t], D_mc_rc[t + 1])
    
    beta[t, 5] ~ dgamma(beta_alpha, beta_theta)
...
# create beta initial values
beta_init <- matrix(NA, nrow = max(additionalt[, 2] + 1), ncol = 5)
for (i in 1:4) {
  beta_init[1:(additionalt[i, 2] + 1), i] <- rep(0.001, additionalt[i, 2] + 1)
}
beta_init[1:constants$n_time_recap, 5] <- rep(0.001, constants$n_time_recap)
...
inits <- function() {
  list(
    ...
    sigma_lambda = 0.3, beta = beta_init,
...
# save samples
saveRDS(out, "data/posterior_samples/savedsamples_IPM_20250818b.rds")
