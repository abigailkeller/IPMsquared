...
model_code <- nimbleCode({
  ...
  for (y in 2:n_year) {
    ...
    ## natural survival
    S[1, y, 1:n_size] <- survival(alpha, exp(log_beta[y]), x[1:n_size], 0, D[1, y])
  }
  ...
  for (y in 1:n_year) {
    for (t in 1:(totalt[y]-1)) {
      ...
      # Equation 7
      ## natural survival
      S[t + 1, y, 1:n_size] <- survival(alpha, exp(log_beta[y]), x[1:n_size], 
                                        D[t, y], D[t + 1, y])
  ...
  for (y in 1:(n_year + 1)) {
    log_beta[y] ~ dnorm(beta_mu, sd = beta_sd)
  }
  ...
  for (y in 1:n_year_short) {
    for (t in additionalt[year_short[y], 1]:additionalt[year_short[y], 2]) {
      ...
      S[t + 1, year_short[y], 1:n_size] <- survival(alpha, exp(log_beta[y]), 
                                                    x[1:n_size],
                                                    D[t, year_short[y]],
                                                    D[t + 1, year_short[y]])
    }
  ...
  ## natural survival
  S[1, 1, 1:n_size] <- survival(alpha, exp(log_beta[1]), x[1:n_size], 
                                0, D[1, 1])
  ...
  S_mc_rc[1, 1:n_size] <- survival(alpha, exp(log_beta[5]), x[1:n_size], 
                                   D_mc_rc[1], D_mc_rc[2])
  ...
  for (t in 2:n_time_recap) { # time corresponds to marking events
    ...
    S_mc_rc[t, 1:n_size] <- survival(alpha, exp(log_beta[5]), x[1:n_size], 
                                     D_mc_rc[t], D_mc_rc[t + 1])
  ...
  # normal distribution mean for log-scale instantaneous intensity of mortality
  beta_mu ~ dunif(-100, 100)
  # normal distribution sd for log-scale instantaneous intensity of mortality
  beta_sd ~ dunif(0, 10000)
...
inits <- function() {
  list(
    ...
    beta_mu = runif(1, log(0.2), log(2)), beta_sd = runif(1, 1, 50), 
    alpha = 0.1, alpha_o_mu = log(0.01), alpha_o_sd = 0.5, gk = 1,
    ...
    sigma_lambda = 0.3, log_beta = rep(log(0.001), 5),
...
out <- clusterEvalQ(cl, {
  ...
  mcmcConf_myModel <- configureMCMC(
    ...
    monitors = c("h_M_max", "h_M_A", "h_M_sigma", "h_F_max",
                 ...
                 "lambda_R", "lambda_A", "log_beta", "alpha_o",
                 "alpha", "beta_mu", "beta_sd",
...
# save samples
saveRDS(out, "data/posterior_samples/savedsamples_IPM_20250814.rds")
