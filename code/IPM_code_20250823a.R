...
model_code <- nimbleCode({
  ...
  # natural survival ranef
  for (i in 1:(n_year + 1)) {
    eps_y[i] ~ dnorm(0, sd = alpha_sd)
  }
  ...
  for (y in 1:(n_year - 1)) {
    ...
    S_o[y, 1:n_size] <- overwinter_survival(alpha_o,
                                            wgrowth_N_sum[y],
                                            x[1:n_size], eps_y_o[y])

    # Equation 11
    ## year-specific intensity of overwinter mortality
    eps_y_o[y] ~ dnorm(0, sd = alpha_o_sd)
  }
  ...
  ## natural survival
  S[1, 1, 1:n_size] <- survival(alpha, beta, x[1:n_size], 0, D[1, 1], eps_y[1])
...
inits <- function() {
  list(
    ...
    alpha_o_sd = 0.5, eps_y_o = c(0, 0, 0), alpha_sd = 0.5,
    eps_y = rep(0, 5), gk = 1,
...
out <- clusterEvalQ(cl, {
  ...
  survival <- nimbleFunction (
    ...
    run = function(alpha = double(0), beta = double(0), x = double(1),
                   t1 = double(0), t2 = double(0), eps_y = double(0))
    {
      returnType(double(1))
      ...
      # get survival rate
      out <- exp(-deltat * (beta + alpha / x ^ 2 + eps_y))
  ...
  overwinter_survival <- nimbleFunction (
    ...
    run = function(alpha_o = double(0), N_sum = double(0),
                   x = double(1), eps_y_o = double(0))
    {
      returnType(double(1))
      
      # get probability of survival
      out <- exp(-(alpha_o * N_sum / x ^ 2 + eps_y_o))
  ...
  mcmcConf_myModel <- configureMCMC(
    ...
    monitors = c("h_M_max", "h_M_A", "h_M_sigma", "h_F_max",
                 ...
                 "alpha", "eps_y_o", "eps_y", "alpha_sd",
                 "alpha_o_sd", "N_overwinter",
  ...
  mcmcConf_myModel$removeSamplers(c("eps_y", "beta"))	
  mcmcConf_myModel$addSampler(c("eps_y[1]", "eps_y[2]", "eps_y[3]",
                                "eps_y[4]", "eps_y[5]", "beta"),
                              type = "RW_block")
...
# save samples
saveRDS(out, "data/posterior_samples/savedsamples_IPM_20250823a.rds")
