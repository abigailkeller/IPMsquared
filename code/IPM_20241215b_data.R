...
# import data
counts <- readRDS('IPM/data/saved/counts_subset9.rds')
ncap <- readRDS('IPM/data/saved/ncap_subset9.rds')
index <- readRDS('IPM/data/saved/index_subset9.rds')
index_full <- readRDS('IPM/data/saved/index_full_subset9.rds')
totalt <- readRDS('IPM/data/saved/totalt_subset9.rds')
totalo <- readRDS('IPM/data/saved/totalo_subset9.rds')
soak_days <- readRDS('IPM/data/saved/soak_days_subset9.rds')
m_index <- readRDS('IPM/data/saved/m_index_subset9.rds')
f_index <- readRDS('IPM/data/saved/f_index_subset9.rds')
s_index <- readRDS('IPM/data/saved/s_index_subset9.rds')

# get mark-recapture data
n1_mc <- readRDS('mark-recapture/n1_mc.rds')
n2_mc <- readRDS('mark-recapture/n2_mc.rds')
m2_mc <- readRDS('mark-recapture/m2_mc.rds')
totalo_mc <- length(1:4)*15*6
soak_days_mc <- rep(1,totalo_mc)
f_index_mc <- rep(1,totalo_mc)
s_index_mc <- rep(0,totalo_mc)
m_index_mc <- rep(0,totalo_mc)
mc_index <- c((198-59)/365,(213-59)/365)

# replace NAs with 0
counts[is.na(counts)] <- 0
ncap[is.na(ncap)] <- 0
totalo[is.na(totalo)] <- 0
soak_days[is.na(soak_days)] <- 0
m_index[is.na(m_index)] <- 0
f_index[is.na(f_index)] <- 0
s_index[is.na(s_index)] <- 0

# fix index
index <- index-3
index_full <- index_full-3

# convert index to fraction
...
code_inter_annual <- nimbleCode({
  ...
  nmort_shape_s ~ dunif(0,50) # gamma distributions shape for instantaneous probability of mortality (natural)
  nmort_rate_s ~ dunif(0,150) # gamma distributions rate for instantaneous probability of mortality (natural)
  nmort_size ~ dunif(0,10000) # size-dependent overwinter natural mortality, shared across all sites
  wnmort_shape_s ~ dunif(0,50) # gamma distributions shape for instantaneous probability of overwinter mortality (natural)
  wnmort_rate_s ~ dunif(0,150) # gamma distributions rate for instantaneous probability of overwinter mortality (natural)
  ...
  for(i in 1:(nsite-1)){
    ...
    wgrowth_N_sum[i] <- sum(wgrowth_N[i,1:nsize])
    
    for(k in 1:nsize){
      N_overwinter[i,k] ~ dbinom(size = round(wgrowth_N[i,k]), 
                                 prob = exp(-(wnmortality_s[i]*wgrowth_N_sum[i]/y[k]^2)))
    }
  }
...
# create recruit_intro structure
recruit_index <- 6
recruit_intro <- matrix(NA,nrow=dim(index)[1],ncol=dim(index)[2])
for(i in 1:dim(index)[1]){
  for(j in 1:dim(index)[2]){
    recruit_intro[i,j] <- ifelse(index[i,j]==recruit_index, 1, 0)
  }
}

# (totalt[i]+1):additionalt[i]
# data structure for additional time for size-independent mortality
additionalt <- cbind(as.vector(totalt),
                     totalt+max(index,na.rm=TRUE) - apply(index, 2, max, na.rm = TRUE)-1)
...
constants <- list(nsite = dim(counts)[3], # number of sites
                  ...
                  site_short = which(apply(index, 2, max, na.rm = TRUE) < max(index,na.rm=TRUE)),
                  nsite_short = length(which(apply(index, 2, max, na.rm = TRUE) < max(index,na.rm=TRUE))),
...
# save samples
saveRDS(out,'20241224_savedsamples.rds')
