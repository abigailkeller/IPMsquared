...
model_code <- nimbleCode({
  ...
  for (y in 2:n_year) {
    ...
    ## natural survival
    S[1, y, 1:n_size] <- survival(alpha[y], x[1:n_size], 0, D[1, y])
  }
  ...
  for (y in 1:n_year) {
    for (t in 1:(totalt[y]-1)) {
      ...
      # Equation 7
      ## natural survival
      S[t + 1, y, 1:n_size] <- survival(alpha[y], x[1:n_size], 
                                        D[t, y], D[t + 1, y])
  ...
  for (y in 1:n_year_short) {
    for (t in additionalt[year_short[y], 1]:additionalt[year_short[y], 2]) {
      ...
      S[t + 1, year_short[y], 1:n_size] <- survival(alpha[y],
                                                    x[1:n_size],
                                                    D[t, year_short[y]],
                                                    D[t + 1, year_short[y]])
    }
  ...
  ## natural survival
  S[1, 1, 1:n_size] <- survival(alpha[1], x[1:n_size], 
                                0, D[1, 1])
  ...
  S_mc_rc[1, 1:n_size] <- survival(alpha[5], x[1:n_size], 
                                   D_mc_rc[1], D_mc_rc[2])
  ...
  for (t in 2:n_time_recap) { # time corresponds to marking events
    ...
    S_mc_rc[t, 1:n_size] <- survival(alpha[5], x[1:n_size], 
                                     D_mc_rc[t], D_mc_rc[t + 1])
  ...
  # lognormal distribution mu - instantaneous prob of mortality
  alpha_mu ~ dunif(-50, 50)
  # lognormal distribution sd - instantaneous prob of mortality
  alpha_sd ~ dunif(0, 150)
...
inits <- function() {
  list(
    ...
    alpha_mu = runif(1, log(5), log(12)), alpha_sd = runif(1, 0.1, 0.5), 
    ...
    sigma_lambda = 0.3, alpha = rep(9, 5),
...
out <- clusterEvalQ(cl, {
  ...
  survival <- nimbleFunction (
    ...
    run = function(alpha = double(0), x = double(1),
...
# save samples
saveRDS(out, "data/posterior_samples/savedsamples_IPM_20250815.rds")
