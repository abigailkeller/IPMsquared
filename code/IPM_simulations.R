...
# size-selectivity -- gaussian function
...
# size-selectivity -- logistic function
...
calc_cond_prob <- function(nobs, nsize, hazard){
  #create empty array
  array <- array(NA,dim=c(nobs,nsize))
  #loop through sizes
  for(k in 1:nsize){
...
# calculating capture probabilities
calc_prob <- function(nobs, nsize, hazard){
  #create empty array
  p <- rep(NA,nsize)
  #loop through sizes
  for(k in 1:nsize){
...
gkernel <- function(params, t1, t2,
  ...
  S_t <- (params$growth_C*params$growth_k/(2*pi))*sin(2*pi*(t2-(1+params$growth_ts)))
  S_t0 <- (params$growth_C*params$growth_k/(2*pi))*sin(2*pi*(t1-(1+params$growth_ts)))
  
  #p(y'|y)
  for(i in 1:nsize){
    increment <- (params$growth_xinf-y[i])*(1-exp(-params$growth_k*(t2-t1)-S_t+S_t0))
    mean <- y[i] + increment
    array[1:nsize,i] <- (pnorm(upper,mean,
                               sd=params$growth_sd)-pnorm(lower,mean,
                                                          sd=params$growth_sd)) 
  }
...
# initial sizes -- lognormal
init_sizes_lnorm <- function(params, lower, upper){
  out <- plnorm(q=upper,meanlog=params$init_lmean_adult,
                sdlog=params$init_lsd_adult)-plnorm(q=lower,
                                                    meanlog=params$init_lmean_adult,
...
simulate_year <- function(params, constant, nobs, N, N_recruit, nmort, 
  ...
  hazard <- calc_hazard(params, constant, nobs, 
                        obs_ref_f, obs_ref_s, obs_ref_m) 
  # p(capture at all)
  p <- calc_prob(nobs, constant$nsize, hazard)
  
  # create vector for captured
  ncap <- rep(NA,constant$nsize)
  for(t in 1:(length(constant$t_index)-1)){
    
    
    # remove crabs
    for(k in 1:constant$nsize){
    ...
    kernel <- gkernel(params,
                      ...
                      constant$nsize,constant$y,
                      constant$lower,constant$upper)
    
    # project to next time period
    if(t == constant$recruit_intro){
      N <- kernel%*%(as.numeric(N)*exp(-(nmort+params$nmort_size/constant$y^2))-ncap) + init_recruit
    } else {
      N <- kernel%*%(as.numeric(N)*exp(-(nmort+params$nmort_size/constant$y^2))-ncap)
    }
  ...
  N_overwinter <- rep(NA,constant$nsize)
  ...
  for(k in 1:constant$nsize){
    ...
    N_overwinter[k] <- rbinom(n = 1,
                              size = new_N, 
                              prob = exp(-(wnmortality_s*wgrowth_N_sum/constant$y[k]^2)))
...
simulate_year_noeffort <- function(params, constant, N, N_recruit, nmort, 
  ...
  for(t in 1:(length(constant$t_index)-1)){
    ...
    # project to next time period
    if(t == constant$recruit_intro){
      N <- kernel%*%(as.numeric(N)*exp(-(nmort+params$nmort_size/constant$y^2))) + init_recruit
    } else {
      N <- kernel%*%(as.numeric(N)*exp(-(nmort+params$nmort_size/constant$y^2)))
    }
...
simulate_interannual <- function(params, constant, nobs,
                                 nyears, nyears_burn, N, N_recruit, 
                                 nmort, wnmortality_s,
                                 obs_ref_f, obs_ref_s, obs_ref_m){
  
  out <- as.data.frame(matrix(NA,nrow=nyears,ncol=length(N)))
  
  for(i in 1:nyears){
    out[i,] <- simulate_year(params, constant, nobs, N, N_recruit[i],
                             nmort[i], wnmortality_s[i], i, nyears,
                             obs_ref_f, obs_ref_s, obs_ref_m)
  }
  
  # subset to averaged years
  sub <- out[(nyears_burn+1):nyears,]
  return(colMeans(sub))
}
simulate_interannual_noeffort <- function(params, constant, nyears, nyears_burn,
                                          N, N_recruit, nmort, wnmortality_s){
...
out <- matrix(NA,nrow=0,ncol=constant$nsize+5)
colnames(out) <- c(1:constant$nsize,
...
N_recruit <- as.data.frame(matrix(nrow=length(index), ncol=constant$nyears))
for(i in 1:length(index)){
  params <- get_params(out_sub,i)
  N[i,] <- init_sizes_lnorm(params,constant$lower,
                            constant$upper)*params$N_adult
  N_recruit[i,] <- rnorm(constant$nyears,params$N_mu_recruit,params$N_sd_recruit)
}
# get nmortality params
nmort <- as.data.frame(matrix(nrow=length(index), ncol=constant$nyears))
wnmortality_s <- as.data.frame(matrix(nrow=length(index), ncol=constant$nyears))
for(i in 1:length(index)){
  params <- get_params(out_sub,i)
  nmort[i,] <- rgamma(constant$nyears,shape=params$nmort_shape_s,
                      rate=params$nmort_rate_s)
  wnmortality_s[i,] <- rgamma(constant$nyears,shape=params$wnmort_shape_s,
...
for(e in effort){
  for(i in 1:length(prop)){
    ...
    for(k in 1:length(index)){
      out <- rbind(out,
                   c(simulate_interannual(get_params(out_sub,index[k]), constant, 
                                          nobs, constant$nyears, constant$nyears_burn,
                                          N[k,], N_recruit[k,],
                                          as.numeric(nmort[k,]), 
...
colnames(out_noeffort) <- 1:constant$nsize
for(i in 1:length(index)){
  out_noeffort[i,] <- simulate_interannual_noeffort(get_params(out_sub,index[i]), constant, 
                                                    constant$nyears, constant$nyears_burn,
                                                    N[i,], N_recruit[i,],
                                                    as.numeric(nmort[i,]), 
