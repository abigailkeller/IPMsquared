...
# read in mark-recapture constants - Seadrift Lagoon
D_mc_sl <- readRDS("data/model_data/mc_index.rds")
totalo_mc_sl <- readRDS("data/model_data/totalo_mc.rds")
soak_days_mc_sl <- readRDS("data/model_data/soak_days_mc.rds")
f_index_mc_sl <- readRDS("data/model_data/f_index_mc.rds")
s_index_mc_sl <- readRDS("data/model_data/s_index_mc.rds")
m_index_mc_sl <- readRDS("data/model_data/m_index_mc.rds")
...
model_code <- nimbleCode({
  ...
  for (k in 1:n_size) {
    m2[k] ~ dbinom(size = round(n1_project[k]), prob = p_mc_sl[k])
  }

  # Equation 25
  ## total capture probability with mark-recapture data
  p_mc_sl[1:n_size] <- calc_prob(totalo_mc_sl, n_size,
                                 hazard_mc_sl[1:totalo_mc_sl, 1:n_size])

  # size-dependent hazard rates with mark-recapture data
  hazard_mc_sl[1:totalo_mc_sl, 1:n_size] <- calc_hazard(
    totalo_mc_sl, n_size, h_F_max, h_F_k, h_F_0, h_S_max, h_S_k, h_S_0, h_M_max,
    h_M_A, h_M_sigma, f_index_mc_sl[1:totalo_mc_sl], 
    s_index_mc_sl[1:totalo_mc_sl],
    m_index_mc_sl[1:totalo_mc_sl], soak_days_mc_sl[1:totalo_mc_sl], x[1:n_size]
  )

  # Equation 26
  ## apply kernel from time of marking to time of recapture
  n1_project[1:n_size] <- get_kernel(xinf, gk, sigma_G, A, ds, D_mc_sl[1], 
                                     D_mc_sl[2],
                                     n_size, pi, x[1:n_size], lower[1:n_size],
                                     upper[1:n_size], S_mc_sl[1:n_size]) %*%
    n1[1:n_size]

  S_mc_sl[1:n_size] <- survival(alpha, beta[5], x[1:n_size], 
                                D_mc_sl[1], D_mc_sl[2])
  ...
  S_mc_rc[1, 1:n_size] <- survival(alpha, beta[6], x[1:n_size], 
                                   D_mc_rc[1], D_mc_rc[2])
  ...
  for (t in 2:n_time_recap) { # time corresponds to marking events
    ...
    S_mc_rc[t, 1:n_size] <- survival(alpha, beta[6], x[1:n_size], 
                                     D_mc_rc[t], D_mc_rc[t + 1])
...
constants <- list(
  ...
  # SL: total number of trap observations in mark-recapture dataset
  totalo_mc_sl = totalo_mc_sl,
  # SL: binary indicator of fukui traps in trap j of mark-recapture dataset
  f_index_mc_sl = f_index_mc_sl,
  # SL: binary indicator of shrimp traps in trap j of mark-recapture dataset
  s_index_mc_sl = s_index_mc_sl,
  # SL: binary indicator of minnow traps in trap j of mark-recapture dataset
  m_index_mc_sl = m_index_mc_sl,
  # SL: number of soak days in trap j of mark-recapture dataset
  soak_days_mc_sl = soak_days_mc_sl,
  # SL: calendar date indices (fraction of year) of mark-recapture dataset
  D_mc_sl = D_mc_sl,
...
data <- list(
  ...
  n1 = n1_mc, # count of marked crabs of size x in SL mark-recapture dataset
  m2 = m2_mc, # count of recaptured crabs of size x in SL mark-recapture dataset
  marked_rc = marked_rc, # count of marked crabs at time t of size x in RC mark-recapture dataset
  catch_rc = catch_rc # count of recaptured crabs at time t of size x in RC mark-recapture dataset
)
...
inits <- function() {
  list(
    ...
    sigma_lambda = 100, beta = rep(0.001, 6),
...
# save samples
saveRDS(out, "code/model_selection/savedsamples_model1_all_mc.rds")
