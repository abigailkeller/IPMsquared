...
inits <- function() {
  list(
    ...
    h_F_max = runif(1, 0.0001, 0.0008), h_F_max_adj = 0,
    h_F_k = 0.2, h_F_0 = 45,
...
out <- clusterEvalQ(cl, {
  ...
  calc_hazard <- nimbleFunction (
                   ...
                   x = double(1))
    {
      ...
      shrimp <- as.integer(sum(obs_ref_s) > 1)
        
      # loop through observations
      for (j in 1:nobs) {
        # P(capture)
        array[j, 1:n_size] <- (
          size_sel_log(pmax = (h_F_max_adj * shrimp + h_F_max), 
...
# save samples
saveRDS(out, "code/model_selection/savedsamples_test4.rds")
...
# read in samples
samples <- readRDS("code/model_selection/savedsamples_test4.rds")
