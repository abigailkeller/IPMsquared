...
model_code <- nimbleCode({
  ...
  for (y in 1:(n_year - 1)) {
    ...
    S_o[y, 1:n_size] <- overwinter_survival(alpha_o_pmax[y], wgrowth_N_sum[y],
                                            alpha_o_xmax[y], alpha_o_sigma[y],
                                            alpha_o_inter, x[1:n_size])

    # Equation 12
    ## year-specific intensity of overwinter mortality
    alpha_o_pmax[y] ~ T(dnorm(over_pmax_mean, sd = over_pmax_sd), 0, Inf)
    alpha_o_xmax[y] ~ T(dnorm(over_xmax_mean, sd = over_xmax_sd), 0, Inf)
    alpha_o_sigma[y] ~ T(dnorm(over_sigma_mean, sd = over_sigma_sd), 0, Inf)
  }
  ...
  over_pmax_mean ~ dunif(0, 500)
  # overwinter mortality
  over_pmax_sd ~ dunif(0, 500)
  # overwinter mortality
  over_xmax_mean ~ dunif(0, 500)
  # overwinter mortality
  over_xmax_sd ~ dunif(0, 500)
  # overwinter mortality
  over_sigma_mean ~ dunif(0, 500)
  # overwinter mortality
  over_sigma_sd ~ dunif(0, 500)
  alpha_o_inter ~ dunif(0, 500)
...
inits <- function() {
  list(
    ...
    alpha_o_pmax = c(0.0001, 0.0005, 0.0005), N_overwinter = N_overwinter,
    alpha_o_xmax = rep(80, 3), alpha_o_sigma = rep(100, 3),
    over_pmax_mean  = 0.02, over_pmax_sd = 0.01, over_xmax_mean = 80,
    over_xmax_sd = 5, over_sigma_mean  = 5, over_sigma_sd = 1,
    alpha_o_inter = 0.1
  )
}
...
out <- clusterEvalQ(cl, {
  ...
  overwinter_survival <- nimbleFunction (
    ...
    run = function(alpha_o_pmax = double(0), N_sum = double(0),
                   alpha_o_xmax = double(0), alpha_o_sigma = double(0),
                   alpha_o_inter = double(0), x = double(1))
    {
      returnType(double(1))
      
      # get probability of survival
      pmax <- alpha_o_pmax * N_sum
      out <- pmax * exp(-(x - alpha_o_xmax) ^ 2 / alpha_o_sigma ^ 2) + 
        alpha_o_inter
  ...
  mcmcConf_myModel <- configureMCMC(
    ...
    monitors = c("h_M_max", "h_M_A", "h_M_sigma", "h_F_max",
                 ...
                 "lambda_R", "lambda_A", "beta", 
                 "alpha", "beta_alpha", "beta_theta",
                 "alpha_o_pmax", "N_overwinter",
                 "alpha_o_xmax", "alpha_o_sigma", "over_pmax_mean",
                 "over_pmax_sd", "over_xmax_mean", "over_xmax_sd",
                 "over_sigma_mean", "over_sigma_sd", "alpha_o_inter",
...
# save samples
saveRDS(out, "code/model_selection/savedsamples_test2.rds")
...
# read in samples
samples <- readRDS("code/model_selection/savedsamples_test2.rds")
...
param1 <- "alpha_o_pmax[1]"
param2 <- "alpha_o_sigma[1]"
param3 <- "alpha_o_xmax[1]"
ggplot() + 
  geom_point(aes(x = samples_mat[, param1], y = samples_mat[, param2],
                 color = samples_mat[, param3]))

chain <- 4
iter <- 1

y <- overwinter_survival(alpha_o_pmax = samples_sub[[chain]][iter, "alpha_o_pmax[3]"], 
                    N_sum = samples_sub[[chain]][iter, "wgrowth_N_sum[3]"],
                    alpha_o_xmax = samples_sub[[chain]][iter, "alpha_o_xmax[3]"], 
                    alpha_o_sigma = samples_sub[[chain]][iter, "alpha_o_sigma[3]"],
                    alpha_o_inter = samples_sub[[chain]][iter, "alpha_o_inter"], 
                    x = x)
plot(x, y)
exp(-(a / x ^ b))

plot(x, exp(-(1 / x ^ 0)))
plot(x, exp(-(1000 / x ^ 2 + 0.1 / x)))
plot(x, exp((0.01 * (1 / x + x))))
