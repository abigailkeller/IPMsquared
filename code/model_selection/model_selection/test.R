  ...
  for (y in 1:(n_year - 1)) {
    ...
    for (k in 1:n_size) {
      N_overwinter[y, k] ~ dbinom(size = round(wgrowth_N[y, k]),
                                  prob = S_o[y, k])
      S_o[y, k] ~ dgamma(alpha_o_alpha, alpha_o_theta)
    }
  }
...
S_o <- matrix(0.5, nrow = dim(C)[3] - 1, ncol = dim(C)[4])
...
inits <- function() {
  list(
    ...
    sigma_lambda = 100, beta = rep(0.001, 5), S_o = S_o,
...
clusterExport(cl, c("model_code", "inits", "data", 
                    "constants", "N_overwinter", "S_o"))
...
# save samples
saveRDS(out, "code/model_selection/savedsamples_test.rds")
...
# read in samples
samples <- readRDS("code/model_selection/savedsamples_test.rds")
...
year1 <- as.data.frame(S_o[, seq(1, 66, 3)])
colnames(year1) <- c(1:22)
year1 <- year1 %>% 
  mutate(index = 1:nrow(S_o[, seq(1, 66, 3)])) %>% 
  pivot_longer(cols = -index, names_to = "x", values_to = "y")

ggplot(data = year1) + 
  geom_line(aes(x = as.numeric(x), y = y,
                group = index), alpha = 0.1) +
  scale_x_continuous(breaks = unique(as.numeric(year1$x)),
                     labels = x)

year3 <- as.data.frame(S_o[, seq(3, 66, 3)])
colnames(year3) <- c(1:22)
year3 <- year3 %>% 
  mutate(index = 1:nrow(S_o[, seq(1, 66, 3)])) %>% 
  pivot_longer(cols = -index, names_to = "x", values_to = "y")
...
ggplot(data = year3) + 
  geom_line(aes(x = as.numeric(x), y = y,
                group = index), alpha = 0.1) +
  scale_x_continuous(breaks = unique(as.numeric(year3$x)),
                     labels = x)
