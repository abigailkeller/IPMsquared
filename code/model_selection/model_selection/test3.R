...
model_code <- nimbleCode({
  ...
  for (y in 1:(n_year - 1)) {
    ...
    alpha_o[y] ~ T(dnorm(alpha_o_mean, sd = alpha_o_sd), 0, Inf)
    alpha_o_exp[y] ~ T(dnorm(alpha_o_exp_mean, sd = alpha_o_exp_sd), 0, Inf)
  }
...
inits <- function() {
  list(
    ...
    alpha_o_mean = 0.01, alpha_o_sd = 0.01, alpha_o_exp = rep(2, 3),
    alpha_o_exp_mean = 2, alpha_o_exp_sd = 1
  )
}
...
out <- clusterEvalQ(cl, {
  ...
  overwinter_survival <- nimbleFunction (
    ...
    run = function(alpha_o = double(0), N_sum = double(0),
                   alpha_o_exp = double(0),
                   x = double(1))
    {
      returnType(double(1))
      
      # get probability of survival
      out <- exp(-(alpha_o * N_sum / x ^ alpha_o_exp))
  ...
  mcmcConf_myModel <- configureMCMC(
    ...
    monitors = c("h_M_max", "h_M_A", "h_M_sigma", "h_F_max",
                 ...
                 "lambda_R", "lambda_A", "beta", "alpha_o", "alpha_o_exp",
                 "alpha", "beta_alpha", "beta_theta",
                 "alpha_o_mean", "alpha_o_sd", 
                 "alpha_o_exp_mean", "alpha_o_exp_sd", "N_overwinter",
...
# save samples
saveRDS(out, "code/model_selection/savedsamples_test3.rds")
...
# read in samples
samples <- readRDS("code/model_selection/savedsamples_test3.rds")
