...
for(i in 1:dim(out)[1]){
  ...
  difference[i,23:(length(colnames(out))-1)] <- out[i,23:(length(colnames(out))-1)]
}
...
for(i in 1:dim(out)[1]){
  ...
  proportion[i,23:(length(colnames(out))-1)] <- out[i,23:(length(colnames(out))-1)]
}
...
effort_summary <- as.data.frame(proportion) %>% 
  ...
  summarise(median = median(p),
            ...
            upper_sd2 = median+sd(p)/2,
            lower_sd = ifelse(median-sd(p)>0,
                              median-sd(p),0),
            lower_sd2 = ifelse(median-sd(p)/2>0,
                               median-sd(p)/2,0)) %>% 
...
final_plot <- plot_112 + plot_560 + plot_2800 + legend +
  plot_layout(nrow=1)
ggsave('figures/supplemental_fig_IPM_simulations_diff.png',dpi=400,width=7,height=3)
