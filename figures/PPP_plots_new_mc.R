...
# read in data
deviance_Drep <- readRDS("data/posterior_predictive_check/new_mc/deviance_Drep.rds")
deviance_D <- readRDS("data/posterior_predictive_check/new_mc/deviance_D.rds")
...
for(i in 1:14) {
  file <- paste0("data/posterior_predictive_check/new/PPS_",i,".rds")
  input <- readRDS(file)
  total <- ncol(input)
