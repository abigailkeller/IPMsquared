...
short_names <- c("mu_A", "sigma_A", "mu_R", "sigma_R", "xinf", "gk", 
                 "A", "ts", "sigma_G", "beta[1]", "beta[2]", "beta[3]", 
...
real_names <- c("log(mu^A)", "sigma^A", "mu^R", "sigma^R",
                "x[infinity]", "k", "A", "t[s]", "sigma^G", "beta[1]",
...
latex_names <- c("log(\\mu^A)", "\\sigma^A", "\\mu^R", "\\sigma^R",
                 "x_{\\infty}", "k", "A", "t_s", "\\sigma^G", "\\beta_1",
...
for (i in seq_len(nrow(posterior))) {
  if (posterior[i, "short_names"] %in% c("lambda_A", "lambda_R[1]",
    ...
    posterior[i, "upper_ci"] <- round(posterior[i, "upper_ci"], digits = 0)
                                                ...
                                                "h_M_max", "h_K_max",
                                                "h_S_max")) {
    posterior[i, "mean"] <- round(posterior[i, "mean"], digits = 6)
    posterior[i, "sd"] <- round(posterior[i, "sd"], digits = 6)
    posterior[i, "lower_ci"] <- round(posterior[i, "lower_ci"], digits = 6)
    posterior[i, "upper_ci"] <- round(posterior[i, "upper_ci"], digits = 6)
  } else {
    posterior[i, "mean"] <- round(posterior[i, "mean"], digits = 6)
    posterior[i, "sd"] <- round(posterior[i, "sd"], digits = 6)
    posterior[i, "lower_ci"] <- round(posterior[i, "lower_ci"], digits = 6)
