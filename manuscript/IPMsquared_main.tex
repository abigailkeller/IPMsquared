...
The process model describes how the population at Drayton Harbor changes through time due to growth, natural mortality, removal, and annual recruitment (Figure 1A). The following process equations describe the integral projection model that uses a kernel to project the population forward in time based on seasonal size-dependent growth and size-dependent natural survival \parencite{rees2014building}. Over the winter between years, size-dependent natural survival in the kernel becomes density-dependent. We then describe the annual recruitment process and how the time series is initialized with the first population density. These equations detail how the population changes within a year (intra-annual change) and between years (inter-annual change) (Figure 1A). The model tracks the state of the population in terms of its distribution of carapace sizes, $N(x)_{t, y}$, which is the density function of individuals of size $x$ during year $y$ at time $t$. 

\subsubsection*{Integral projection model}

The population density, $N(x)_{t, y}$, is projected forward in time using an integral projection model. The integral projection uses a continuous distribution over $x$, and for estimation, the abundance of individuals is discretized using a small size interval ($\Delta x = 5$). The discretization is centered on size $x$, such that the IPM kernel is constructed using the midpoints of the discrete IPM mesh. The total population size is $\int_{x \in \Omega} N(x)_{t, y}dx$, where $\Omega$ represents all biologically feasible sizes (0 - 110 mm).

A kernel, $K(x', x, D)$, describes the probability density of moving from size $x$ to size $x'$. This kernel is time-dependent, where $D = \{d_t, d_{t+1}\}$, a vector of Julian calendar dates associated with $t$ and $t+1$. $N(x)_{t+1,y}$ is therefore a function of $N(x)_{t,y}$, $K(x', x, T)$, the total count of removed crabs in each time period, $C^T(x)_{t,y}$, and recruit abundance, $R(x)$. Recruits annually enter the model at time point, $t_R$:

\begin{equation}
N(x)_{t+1,y} = \int_{x' \in \Omega} K(x',x, D) (N(x)_{t,y} - C^T(x)_{t,y}) dx + R(x)I_{t = t_R}
\end{equation}

The kernel is defined as the product of a growth kernel, $G(x',x, D)$ and size-dependent natural survival, $S(x)$:

\begin{equation}
K(x',x, D) = G(x',x, D) \times S(x)
\end{equation}
...
\begin{equation}
\mu^G_{x,D} = x + (x_{\infty}-x)(1-\text{exp}(-k\Delta t-s(d_{t+1})+s(d_t))
\end{equation}
\begin{equation}
s(d_{t+1}) = \frac{Ak}{2\pi} \text{sin}(2\pi(d_{t+1}-t_s))
\end{equation}
\begin{equation}
s(d_t) = \frac{Ak}{2\pi} \text{sin}(2\pi(d_t-t_s))
\end{equation}
where $\mu^G_{x,D}$ is the mean size at $t+1$, $x_{\infty}$ is the asymptotic average size, $k$ is a measure of the exponential rate of approach to the asymptotic size, $A$ modulates the amplitude of the growth oscillations, and $t_s$ is the time between the start of the calendar year and the start of the convex portion of the first sinusoidal growth oscillation \parencite{garcia2012technical}.

To account for variation in growth rate among individuals, $G(x',x, D)$ is described as:

\begin{equation}
G(x',x, T) = \phi(x'; \mu^G_{x, D}, \sigma^G)
\end{equation}
...
\begin{equation}
S(x)_y = \text{exp}\left(-\Delta t(\beta_y+\frac{\alpha}{x^2})\right)
\end{equation}
...
To transition from year $y$ to year $y+1$, the population density experiences seasonal growth and density- and size-dependent overwinter survival. The abundance of crabs in each size interval surviving the winter, $N(x)_{t=1,y+1}$, is drawn from a binomial distribution, where the number of trials is the abundance of crabs after seasonal growth and removal in the last time period, $t_{max}$, and the probability of success is the probability of overwinter survival, $S^o(x)_y$. Here, the date interval, $D$, used in the seasonal growth equation, $G(x',x,D)$, corresponds to the last time point in the year, $t_{max}$, and the first time point in the following year (i.e., $D=\{d_{t=t_{max}}, d_{t=1}\}$). 

\begin{equation}
N(x')_{t,y} = \int_{x' \in \Omega} G(x',x, D) (N(x)_{t_{max},y} - C^T(x)_{t_{max},y})dx
\end{equation}
\begin{equation}
N(x)_{t=1,y+1} \sim \text{Binomial}\left( N(x')_{t,y},  S^o(x)_y\right)
\end{equation}

Due to thermal stress and starvation, the intensity of overwinter mortality is likely stronger than other times of the year and plays an important role in population regulation through density-dependent control on population size \parencite{henderson1988size}. Overwinter mortality is also size-selective; smaller animals tend to have lower energy reserves than larger animals and use reserves more rapidly due to the allometry of metabolic rate \parencite{hurst2007causes}. Probability of overwinter survival, $S^o$, is therefore modeled as a density-size interaction, such that the intensity of size-dependent overwinter mortality increases at higher population densities. $N^T_{t_{max},y}$ is the total abundance in year $i$ at the onset of winter.

\begin{equation}
S^o(x,N^T_{t_{max},y}) = \text{exp}\left(-\frac{\alpha_y^o \times N^T_{t_{max},y}}{x^2}\right)
\end{equation}
...
The first size distribution, $N(x)_{t=1, y=1}$, is defined in terms of the abundance of adults in the first time period during the first year, $\lambda^{A}$, the mean initial adult size in millimeters, $\mu^A$, and the standard deviation of initial adult size in millimeters, $\sigma^A$. Since the first year of time series data (D1) in 2020, $y = 1$, was at the start of green crab establishment in Drayton Harbor, we expected the size distribution to be dominated by age-one crabs with a few age-two crabs representing the first cohort of colonizing crabs. We therefore assumed a log-normal initial density, $N(x)$, to allow for a unimodal size distribution with a longer right tail. Here, $\phi_L$ represents the log-normal probability density function.

\begin{equation}
N(x)_{t=1, y=1} = \phi_L(x; \mu^A, \sigma^A) \times \lambda^A
\end{equation}
...
The annual size distribution of recruits, $R(x)_{y}$, is defined in terms of the annual abundance of recruits, $\lambda^R_y$, the mean initial recruit size in millimeters, $\mu^R$, and the standard deviation of initial recruit size in millimeters, $\sigma^R$. $\phi$ represents the normal probability density function.

\begin{equation}
R(x)_{y} = \phi(x; \mu^R, \sigma^R) \times \lambda^R
\end{equation}
...
A conditional multinomial observation model was used to describe the data-generating process for the time series data at Drayton Harbor (D1, Figure 1). Here, the removal count data, $C(x)_{t,j,y}$, represent the number of crabs of size $x$, caught at time $t$, during year $y$, in trap $j$, conditioned on the size-structured latent abundance, $N(x)_{t,y}$ \parencite{kery2015modeling}. 

Multiple traps were placed simultaneously at each time period, so we factor the probability of the removal count data in one time period as 1) a binomial distribution for the total number of removed crabs, $C^T(x)_{t,y}$ across all traps and 2) a multinomial-Dirichlet distribution to describe which traps the crabs are caught in, given the total number trapped. The total number of removed crabs, $C(x)_{t,y}$, follows a binomial distribution with number of trials equal to the latent total abundance of crabs, $N(x)_{t,y}$, and total capture probability across all traps set during the same time period, $p(x)_{t,y}$.

\begin{equation}
C^T(x)_{t,y} \sim \text{Binomial}(N(x)_{t,y}, p(x)_{t,y})
\end{equation}

The size-structured count of crabs in each trap, $C(x)_{t,j,y}$, follows a multinomial-Dirichlet mixture distribution where the probability of capture in trap $j$, $p^C(x)_{t,j,y}$, is conditioned on being captured at all. Since the trap compositional data are overdispersed due to green crab aggregation and spatial behaviors, the Dirichlet allows for greater variance in the count data than predicted by a multinomial distribution \parencite{thorson2017model}. The parameter $\alpha^D$ describes the amount of over-dispersion.  More information on the multinomial-Dirichlet mixture can be found in Appendix 2.2.

\begin{equation}
C(x)_{t,j,y} | C^T(x)_{t,y} \sim \text{MultinomialDirichlet}(C^T(x)_{t,y}, p^C(x)_{t,j,y}| \alpha^D)
\end{equation}
...
\begin{equation}
H_F(x) = \frac{h^{max}_F}{1+\text{exp}\left\{-h^k_F(x-h^0_F)\right\}}
\end{equation}
\begin{equation}
H_S(x) = \frac{h^{max}_S}{1+\text{exp}\left\{-h^k_S(x-h^0_S)\right\}}
\end{equation}
...
\begin{equation}
H_M(x) = h^{max}_M \times exp(\frac{x-h^{A}_M}{2 h^{\sigma}_M})
\end{equation}

Each baited trap, $j$, is placed in the habitat for a short ($\sim$24-48 hr) time interval, $\Delta b_{t,j,y}$. The total capture probability, $p(x)_{t,y}$, is described as the probability of not surviving the trapping time interval, $p(x)_{t,y}$. The probability of surviving the trap interval is the integrated hazard rate, summed across all traps set during the same time period, $O_{t,y}$.

\begin{equation}
p(x)_{t,y} = 1-\text{exp}\left(-\sum_{j=1}^{O_{t,y}} H(x)_{t,j,y}\Delta b_{t,j,y}\right)
\end{equation}

The conditional probability of capture, $p^C(x)_{t,j,y}$, is then $H(x)_{t,j,y}/\sum_{j=1}^{O_{t,y}}H(x)_{t,j,y}$.

\subsubsection*{Size-at-age observation process (D2)}

The size-at-age data, $W(a)_{y}$, indicate the carapace width of an individual crab at age $a$ in year $y$. These data are fit to a similar seasonal growth model as in Equations 3-5, except instead of describing incremental growth from size $x$ to $x'$, the below growth equation describes growth from the age of the crab when it is of size 0, $t_0$, to the expected size of a crab at age $a$, $\widetilde{W(a)}$. A normally distributed error term, $\epsilon_i$, is used to account for non-independence among data collected in the same year, since growth rate is likely affected by water temperature, which varies from year-to-year.

\begin{equation}
\widetilde{W(a)}_{i} = x_{\infty}(1-exp(-k(a-t_0) - s(a) + s(t_0))) + \epsilon_i
\end{equation}

To account for variation in growth rate, the observed size-at-age data, $W(a)_{i}$, follow a normal distribution, with the expected carapace width $\widetilde{W(a)}_{i}$, and standard deviation, $\tau_{w}$.

\begin{equation}
W(a)_{i} \sim Normal(\widetilde{W(a)}_{i}, \tau_{w})
\end{equation}
...
The batch mark-recapture data consists of 1) the count of marked and released crabs of size $x$ in the first time period, $n^{mc}(y)$, and 2) the count of recaptured and marked crabs of size $y$ in the second time period, $m^{mc}(x)$. These data follow a binomial distribution:

\begin{equation}
m^{mc}(x) \sim \text{Binomial}(n^{mc}(x)_{t_2}, p^{mc}(x)) 
\end{equation}
where $p^{mc}(x)$ is the total probability of capture based on the total number of Fukui traps set, $O^{mc}$, over the time period $\Delta b^{mc}$:

\begin{equation}
p^{mc}(x) = 1-\text{exp}\left(-\sum_{j=1}^{O^{mc}}\int H_F(x)\Delta b^{mc}\right)
\end{equation}

These data also informed components of the growth and natural mortality kernel (Table 1; Figure 2). The number of marked and released crabs, $n^{mc}(x)$, at $t_1^{mc}$ underwent seasonal growth and natural mortality to the next time period, $t_2^{mc}$.

\begin{equation}
n^{mc}(x)_{t_2} = \int_{x \in \Omega} K(x',x, D^{mc}) n^{mc}(x)_{t_1}dx
\end{equation}
...
\caption{Notation and biological meaning of data, latent states, and parameters. Category refers to the parameter categories designated in Figure 2: 1) Init is the size structure of initial population density and annual recruits, 2) Growth is seasonal growth, 2) N. mort is size-dependent and size-independent natural mortality in non-winter months, 3) O. mort is size- and density-dependent overwinter mortality, 4) F obs, M obs, and S obs correspond to the size-selective observation process for Fukui, Minnow, and Shrimp traps, respectively, and 5) Latent corresponds to the latent states in the state-space model (Figure 1).}
 ...
 \hline
 $N(x)_{t,y}$ & Population density function of individuals of size $x$, during year $y$, at time $t$. & Latent \\ 
 \hline
 ...
 \hline
 $C^T(x)_{t,i}$ & Total count of removed crabs across all traps during time $t$, in year $y$, of size $x$ in the time-series dataset (D1). & - \\ 
 \hline
 $C(x)_{t,j,y}$ & Count of removed crabs in time $t$, in trap $j$, in year $y$, of size $x$ in the time-series dataset (D1). & - \\
 \hline
 $O_{t,y}$ & Number of observations (traps) in time $t$, in year $y$ in the time-series dataset (D1). & - \\
 \hline
 $W(a)_{y}$ & Size of crabs of age $a$, during year $y$ in the size-at-age dataset (D2). & - \\
 \hline
 $n^{mc}(x)$ & Number of marked and released crabs of size $x$ in the mark-recapture dataset (D3). & - \\
 \hline
 $m^{mc}(x)$ & Number of recaptured marked crabs of size $x$ in the mark-recapture dataset (D3). & - \\
 \hline
...
\begin{figure}[H]
    ...
    \caption{Size-structured probability of capture, $p_{x}$, in one trap over a 24 hours trapping period. Colors indicate the trap type. Note that the y-axis is presented with a square root transformation.}
\end{figure}
