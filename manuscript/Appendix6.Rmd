---
title: "Appendix 6: Posterior summaries and trace plots"
output: pdf_document
---
# Appendix 6
...
short_names <- c("mu_A_y", "sigma_A_y", "mu_R_y", "sigma_R_y", "yinf", "gk", 
                 "C", "ts", "sigma_G", "beta[1]", "beta[2]", "beta[3]", 
                 ...
                 "lambda_R[4]", "mu_R_lambda", "sigma_R_lambda")
...
latex_names <- c("log(\\mu_y^A)", "\\sigma_y^A", "\\mu_y^R", "\\sigma_y^R",
                 ...
                 "\\lambda_3^R", "\\lambda_4^R", "\\mu_{\\lambda}^R",
                 "\\mu_{\\sigma}^R")
...
# round
for (i in seq_len(nrow(posterior))) {
  if (posterior[i, "short_names"] %in% c("lambda_A", "lambda_R[1]",
                                         "lambda_R[2]", "lambda_R[3]",
                                         "lambda_R[4]")) {
...
**Table A6:** Posterior summaries, including the mean, standard deviation, 95% credibility interval (highest density interval), $\hat{R}$, and effective sample size. Descriptions of parameters can be found in Table 1. More information about parameter $\rho$ can be found in Appendix 2.1.
...
```
\newpage
**Figure A6:** Trace plots of posterior samples. Colors refer to separate chains. Descriptions of parameters can be found in Table 1.
