...
```{r, echo = FALSE, results = 'hide'}
# read in samples
out <- readRDS('20241215b_savedsamples.rds')
lower <- 2000
upper <- 10001
```
...
latex_names <- c('log(\\mu_y^A)', '\\sigma_y^A', '\\mu_y^R', '\\sigma_y^R',
                ...
                '\\beta[\\alpha]', '\\beta[\\theta]', '\\alpha_1^o', 
                '\\alpha_2^o',
...
knitr::kable(
  posterior_final,
  caption = "Table A2: Posterior summaries, including the mean, standard deviation, 95\\% credibility interval (highest density interval), Rhat, and effective sample size. Descriptions of parameters can be found in Table 1.",
  escape = FALSE,
  format = "latex",
  booktabs = TRUE,
  longtable = TRUE
