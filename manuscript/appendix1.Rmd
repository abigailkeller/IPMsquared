...
$$
To account for variation in growth rate, the observed size-at-age data, $W_{a,i}$, follows a normal distribution, with the expected carapace width $\widetilde{W}_{a,i}$, and standard deviation, $\tau_{w}$.
...
out <- clusterEvalQ(cl, {
  ...
  library(coda)
  # build the model
  model_season <- nimbleModel(seasonal_growth, constants, 
                              model_data, inits_season)
  # build the MCMC
