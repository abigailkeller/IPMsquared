...
The size-at-age data, $W(a)_{y}$, indicating carapace width at age $a$ in year $y$ are fit to the same seasonal growth model as in the main text [@somers1988seasonally]. These data are fit to a similar seasonal growth model as in Equations 3-5 in the main text, except instead of describing incremental growth from size $x$ to $x'$, the below growth equation describes growth from the age of the crab when it is of size 0, $t_0$, to the expected size of a crab at age $a$, $\widetilde{W(a)}$. Additionally here, we account for non-independence among data collected in the same year, since growth rate is likely affected by water temperature, which varies from year-to-year.
The expected carapace width, $\widetilde{W(a)}$, of a crab at age $a$ is:

$$
\widetilde{W(a)}_{i} = x_{\infty}(1-exp(-k(a-t_0) - s(a) + s(t0))) + \epsilon_i
$$
$$
s(a) = \frac{Ak}{2\pi}sin(2\pi(a-t_s))
$$

$$
s(t_0) = \frac{Ak}{2\pi}sin(2\pi(t_0-t_s))
$$
...
$$
To account for variation in growth rate, the observed size-at-age data, $W(a)_{i}$, follows a normal distribution, with the expected carapace width $\widetilde{W(a)}_{i}$, and standard deviation, $\tau_{w}$.
...
short_names <- c("A", "k", "xinf", "t0", "tau_w", "tau_y", "ts")
real_names <- c("A", "k", "x[infinity]", "t[0]", "tau[w]", "tau[y]", "t[s]")
latex_names <- c("A", "k", "x_{\\infty}", "t_0", "\\tau_w", "\\tau_y", "t_s")
